...
class Resource(object):
    ...
    def __repr__(self):
        rv = dict(self.subresources)
        rv['resource'] = self.resource
        rv['typeof'] = self.typeof
...
class ResourceStore(object):
    ...
    def register_resource(self, resource):
        ...
        for relation_type in resource.get_relations():
            for subresource in resource.get_subresources(relation_type):
                self.register_resource(subresource)
    ...
    def parse_resource(self, resource_map):
        ...
        self.check_resource_is_IRI(resource_map)
    ...
    def is_relation(self, label, resource_map):
        ...
        # check if subject belongs to vocabulary
        #if resource_map['vocab'] not subject.toPython()
        # check if subject is relation
        if not subject or not self.vocab_store.is_property(subject):
            raise InvalidResourceMapError(message="Illegal resource relationship: %s" % (label))
        return True
    ...
    def check_resource_is_IRI(self, resource_map):
        try:
            parse_IRI(resource_map['resource'])
        except ValueError:
            raise InvalidResourceMapError(message = 'resource identifier is not an IRI: %s' % (resource_map['resource']))
