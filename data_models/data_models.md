...
+ The motivation `classifying` is taken from the [official W3C list of motivations](https://www.w3.org/TR/annotation-model/#motivation-and-purpose). The W3C framework also has a property `purpose` which is only used as the property of a body of type `TextualBody` or `SpecificResource`. I would prefer to use `purpose` as a property on all bodies, since that conveys better what the purpose of the annotation is (and makes more sense with multiple bodies, allowing a different purpose per annotation body, e.g. classifying and commenting). Classifying is not a motivation, i.e., it doesn't say why or in what context I'm classifying the target. 
