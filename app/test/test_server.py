...
class TestAnnotationAPI(unittest.TestCase):
    ...
    def add_example(self, access_status=None):
        ...
        if access_status:
            ...
            response = self.app.post("/api/annotations", query_string=url_params,
                                     data=json.dumps(annotation),
                                     content_type="application/json", headers=self.headers1)
        else:
            response = self.app.post("/api/annotations", data=json.dumps(annotation),
                                     content_type="application/json", headers=self.headers1)
    ...
    def test_PUT_annotation_returns_modified_annotation(self):
        ...
        response = self.app.put("/api/annotations/" + example['id'], data=json.dumps(example),
                                content_type="application/json", headers=self.headers1)
        annotation = get_json(response)
...
class TestAnnotationAPICollectionEndpoints(unittest.TestCase):
    ...
    def add_example(self, access_status=None):
        ...
        if access_status:
            url_params = {"access_status": access_status}
            response = self.app.post("/api/collections", query_string=url_params, data=json.dumps(collection_raw),
                                     content_type="application/json", headers=self.headers1)
        else:
            response = self.app.post("/api/collections", data=json.dumps(collection_raw),
                                     content_type="application/json", headers=self.headers1)
        return get_json(response)
    ...
    def test_api_can_update_collection(self):
        ...
        response = self.app.put("/api/collections/%s" % (collection_registered["id"]),
    ...
    def test_api_can_add_annotation_to_collection(self):
        ...
        response = self.app.post("/api/annotations", data=json.dumps(annotation_raw),
                                 content_type="application/json", headers=self.headers1)
        ...
        self.app.post("/api/collections/%s/annotations/" % (collection_registered["id"]),
    ...
    def test_api_can_get_collection_annotations_via_url_params(self):
        ...
        self.app.post("/api/collections/%s/annotations/" % (collection_registered["id"]),
                      ...
                      content_type="application/json", headers=self.headers1)
        url_params = {"iris": 0}
        response = self.app.get("/api/collections/%s/annotations/" % (collection_registered["id"]),
                                query_string=url_params, headers=self.headers1)
        collection_retrieved = get_json(response)
        items = collection_retrieved["first"]["items"]
        self.assertEqual(isinstance(items[0], dict), True)
    ...
    def test_api_can_remove_annotation_from_collection(self):
        ...
        self.app.post("/api/collections/%s/annotations/" % (collection_registered["id"]),
                      data=json.dumps(annotation_registered),
                      content_type="application/json", headers=self.headers1)
        self.app.delete("/api/collections/%s/annotations/%s" % (collection_registered["id"],
                        annotation_registered["id"]),
...
class TestUserAPI(unittest.TestCase):
    ...
    def test_POST_user_without_password_returns_an_error(self):
        response = self.app.post("/api/users", data=json.dumps({"username": self.testuser}),
                                 content_type="application/json")
        self.assertEqual(response.status_code, 400)

    def test_POST_user_with_password_returns_user_created(self):
        response = self.app.post("/api/users", data=json.dumps({"username": self.testuser, "password": self.testpass}),
                                 content_type="application/json")
    ...
    def test_POST_login_without_password_returns_error(self):
        response = self.app.post("/api/login", data=json.dumps({"username": self.testuser}),
                                 content_type="application/json")
        self.assertEqual(response.status_code, 403)
    ...
    def test_PUT_user_without_old_password_returns_error(self):
        self.register_user()
        response = self.app.put("/api/users", data=json.dumps({"password": "new_pass"}),
    ...
    def test_PUT_unregistered_user_returns_error(self):
        ...
        response = self.app.put("/api/users", data=json.dumps({"username": "unknown", "password": "new_pass"}),
                                content_type="application/json", headers=headers)
        self.assertEqual(response.status_code, 403)

    def test_PUT_user_with_old_and_new_password_returns_password_updated(self):
        self.register_user()
        response = self.app.put("/api/users", data=json.dumps({"password": self.testpass, "new_password": "new_pass"}),
                                content_type="application/json", headers=self.headers)
        response_data = get_json(response)
